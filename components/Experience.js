                ...
                <li class={`bg-opacity-0 experience   backdrop-blur-lg bg-gray-800  ring-1 lg:w-[45%] w-full ring-slate-100/10 rounded-md sm:h-[275px] h-[200px] ${i == 1 ? 'sm:self-end' : 'sm:self start'} self-start`}>            
                  <div className="flex gap-2 align-middle items-center border-b border-[#ffffff20] px-4">
                    ...
                    <div className="flex">
                      {jobs.map((job, idx) => {
                        return (
                            <code className={`cursor-pointer px-1 py-3  text-[8px] sm:text-xs sm:px-3   border-[1px] border-[#ffffff20] border-l-[#00000000] ${i == idx ? 'border-b-black' : 'border-b-[#ffffff00] '} ${job.id == 1 ? 'ml-2 border-l-[#ffffff20]' : ''} border-t-black relative md:top-[0.07em] top-[1em]  flex justify-between gap-1 place-items-center`}>
